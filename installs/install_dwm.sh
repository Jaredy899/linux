...
install_packages() {
    ...
    case "$distro" in
        ubuntu|debian)
            ...
            wget -q https://packages.mozilla.org/apt/repo-signing-key.gpg -O /etc/apt/keyrings/mozilla-archive-keyring.gpg

            echo "deb [signed-by=/etc/apt/keyrings/mozilla-archive-keyring.gpg] https://packages.mozilla.org/apt mozilla main" | sudo tee /etc/apt/sources.list.d/mozilla.list > /dev/null
            echo '
            Package: *

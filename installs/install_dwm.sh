...
setupDWM() {
    ...
    case "$PACKAGER" in
        ...
        dnf)
            ...
            ;;
        zypper)
            $ESCALATION_TOOL zypper install -y xinit xorg-x11-server libX11-devel libXinerama-devel libXft-devel imlib2-devel libxcb-devel libxcb-devel gcc git libconfig-devel libdrm-devel libev-devel libX11-devel libX11-xcb1 libXext-devel libxcb-devel Mesa-libGL-devel Mesa-libEGL-devel libepoxy-devel meson pcre2-devel uthash-devel xcb-util-image-devel libpixman-1-0-devel xcb-util-renderutil-devel xcb-util-devel
            ;;
        *)
            printf "%b\n" "${RED}Unsupported package manager: $PACKAGER${RC}"
            exit 1

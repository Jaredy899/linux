...
setupDWM() {
    ...
    case "$PACKAGER" in
        ...
        dnf)
            ...
            ;;
        zypper)
            $ESCALATION_TOOL "$PACKAGER" install -y xorg-x11-server xinit gcc make dbus-1-devel git libconfig-devel libdrm-devel libev-devel libX11-devel libX11-xcb1 libxcb-devel libGL-devel Mesa-libEGL-devel libepoxy-devel meson pcre2-devel libpixman-1-0-devel uthash-devel xcb-util-image-devel xcb-util-renderutil-devel xorgproto-devel xcb-util-devel
            ;;
        *)
